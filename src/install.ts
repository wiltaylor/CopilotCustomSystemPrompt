...
const extensionRoot = join(copilotRoot, "extensions", "copilot-custom-system-prompt");
...
if (exists && !force) {
  throw new Error(`${extensionRoot} already exists; pass --force to replace it.`);
}
if (exists) {
  await rm(extensionRoot, { recursive: true, force: true });
}
await cp(join(packageRoot, "extensions", "copilot-custom-system-prompt"), extensionRoot, {
  recursive: true,
  force: true,
});
